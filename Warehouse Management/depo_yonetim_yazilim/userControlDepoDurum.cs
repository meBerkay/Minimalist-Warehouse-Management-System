...
namespace depo_yonetim_uygulamasi
{
    public partial class userControlDepoDurum : UserControl
    {
        ...
        public void DepoCekme()
        {
            ...
            SqlDataReader vericek;
            baglanti.Open();
            vericek = komut.ExecuteReader();
            while (vericek.Read())
            {
                comboBox1.Items.Add(vericek["adi"]);
            }
            baglanti.Close();
        }
        ...
        public void MalzemeCekme(String depoadi)
        {
            ...
            while (vericek.Read())
            {
                comboBox2.Items.Add(vericek["ad"]);
            }
            baglanti.Close();
            depoadi = null;
        }
        ...
        public void KritikSeviye()
        {
            ...
            while (vericek.Read())
            {
                kritikseviye.Add(Convert.ToInt32(vericek["kritik_seviye"]));
            }
            baglanti.Close();
        }
        ...
        void Renklendirme()
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count -1; i++)
            {
                DataGridViewCellStyle renk = new DataGridViewCellStyle();
                    ...
                    //DEPO KONTROL KIRMIZI ŞERİT
                    try
                    {
                        
                        if (Convert.ToInt32(value: dataGridView1.Rows[i].Cells["g_miktar"].Value) < kritikseviye[i])
                        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
                ...
                {
                    MessageBox.Show("Bütün bilgileri DOLDURMALISINIZ!", "HATA", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
